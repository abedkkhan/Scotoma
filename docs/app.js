const state={index:null,trace:null,coverage:null,adjudication:null,flip:null,evaluation:null,audited:false,selectedRepoFiles:[],selectedTranscript:null,repoName:'Flask'};
...
$('#repoFolder').addEventListener('change',e=>{
  ...
  locPromise.then(lines=>{$('#uploadStatus').innerHTML=`<b>${escapeHtml(state.repoName)}</b>: ${units.length} source files · ${lines.reduce((a,b)=>a+b,0).toLocaleString()} sampled LOC. ${state.selectedTranscript?'Both inputs ready.':'Now choose the agent session log.'}`;$('#workspaceLabel').textContent=`NEW AUDIT / ${state.repoName.toUpperCase()}`;readyForAudit();toast('Repository territory loaded')});
});
$('#transcriptFile').addEventListener('change',e=>{state.selectedTranscript=e.target.files[0]||null;if(state.selectedTranscript){$('#uploadStatus').innerHTML=`<b>${escapeHtml(state.selectedTranscript.name)}</b> loaded (${(state.selectedTranscript.size/1024).toFixed(1)} KB). ${state.selectedRepoFiles.length?'Both inputs ready.':'Now choose the matching repository.'}`;toast('Agent session log loaded')}readyForAudit()});
...
async function startLiveAudit(){
  ...
  if(!api)return setUploadError('Backend not configured. Deploy render.yaml, then paste its service URL here.');if(question.length<5)return setUploadError('Enter the question that the agent investigated.');if(!state.selectedRepoFiles.length)return setUploadError('Choose the investigated repository first.');if(!state.selectedTranscript)return setUploadError('Choose the matching Claude Code or Codex session log.');
  ...
  const form=new FormData();form.append('question',question);form.append('vendor',$('#agentVendor').value);form.append('transcript',state.selectedTranscript,state.selectedTranscript.name);state.selectedRepoFiles.forEach(file=>{form.append('files',file,file.name);form.append('paths',file.webkitRelativePath||file.name)});
...
function showLiveProgress(job){
  state.audited=false;$('#modeKicker').textContent='IMPORTED SESSION · AGENT COVERAGE ANALYSIS';$('#workspaceLabel').textContent=`AUDITING / ${state.repoName.toUpperCase()}`;$('#questionText').textContent=$('#questionInput').value.trim();$('#originalAnswer').textContent='Scotoma is mapping the repository and reconstructing the uploaded agent session…';$('#auditMessage').classList.add('hidden');$('#correctedMessage').classList.add('hidden');
